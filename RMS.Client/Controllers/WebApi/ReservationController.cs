...
namespace RMS.Client.Controllers.WebApi
{
    ...
    public class ReservationController : ApiController
    {
        ...
        [HttpGet]
        public List<ReservationViewModel> GetUserReservation()
        {
            var user = GetUserByLogin();
            var reservationEntities = rsvManager.Get()
                .Where(r => r.User.Id == user.Id)
                .ToList();
            var reservationList = Mapper.Map<List<ReservationViewModel>>(reservationEntities);
            ...
            List<ReservedTable> lstReservation = restaurant.DinnerTables
                ?.Select(t => new ReservedTable()
                {
                    ...
                    Reservations = t.Reservations
                                     .Where(r => r.From.Day == date.Day
                                                 && r.From.Month == date.Month
                                                 && r.From.Year == date.Year)
